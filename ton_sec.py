...
def toc():
	...
	tel.write(("show standby brief | in "+host_ip[:-2]+"\r").encode('ascii'))
    	hsrp_output = tel.read_until("#".encode('ascii'))
    	tel.write(("show ip eigrp nei | in "+host_ip[:-2]+"\r").encode('ascii'))
	...
	#print("eigrp_splitline",eigrp_splitline)
	if eigrp_splitline and hsrp == "False":
