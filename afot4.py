...
#*************************** commands ***********************
	...
	vrf_output = tel.read_until("#".encode('ascii'))
...
#	print("wan_interfaces",wan_interfaces)	
	if bool(re.search('send-community',bgp_config.decode("utf-8"))) is True:
...
#************************************************** Find out LAN interfaces ************************************
	...
	if len(lan_interfaces) != 0:
			...
			#if bool(re.search(nei_ip,int_out.decode('utf-8'))) is True:
			#	interface = inter
			#	print("wan inter enter")	
	...
	if len(tun_int) == 0 and len(interface) == 0: # to find the interface to shut
...
#*********************************************identify the vrf status and name *********************************************
	...
	#print("to_test",to_test)
		vrf_out_splitline = vrf_output.decode("utf-8").splitlines()[2:]
		vrf_lines = []
    		vrf_lines1 = []
		for vrf_dum in vrf_out_splitline:
        		vrf_dum_split = vrf_dum.split()
			if len(vrf_dum_split) > 2:
            			vrf_lines.append(str(vrf_dum_split).replace("[","").replace("]","").strip())
		for vrf in vrf_lines:
        		vrf_lines1.append(vrf)
		for vrname in vrf_lines1:
        		vfn = vrname.split(",")
        		vrf_names.append(vfn[0].replace("'","").replace("u","").replace("u'","").strip())
