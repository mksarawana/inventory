...
def toc():
	...
	tel.write("sh start | i ntp \r".encode('ascii'))
        ntp = tel.read_until("#".encode('ascii'))
	print("ntp",ntp.decode("utf-8"))
...
def inventory():
	print("inventory is on")	
	tel.write(('sh run | i hostname\r').encode('ascii'))
        hostname_output = tel.read_until("#".encode('ascii'))
	tel.write(('sh ip int brief | exc unassigned\r').encode('ascii'))
        int_brief_output = tel.read_until("#".encode('ascii'))
	

#************************************************************** Host Name Section ****************************************************************************
	hostname_output_splitlin = hostname_output.decode("utf-8").splitlines()[1:][:-1]
	hostname = str(hostname_output_splitlin)[2:].replace("]","").replace("'","").replace("hostname","").strip()
	print(hostname)

#********************************************************** VLAN And IP Address ********************************************************************************
	interface = []
	ip_address = []
	int_out_splitline = int_brief_output.decode("utf-8").strip().splitlines()[1:][:-1]
	#print(int_out_splitline,type(int_out_splitline),len(int_out_splitline))
    	for index,nettt in enumerate(int_out_splitline):
		#print("feGSJFESGFE",nettt,type(nettt),len(nettt))
		if len(nettt) != 0:
        		grap = nettt.split()
			#print(grap[0])
       			interface.append(grap[0])
        		ip_address.append(grap[1])
    	print(interface, ip_address)
    	#dictionary = dict(zip(interface, ip_address))
    	#print(dictionary)
...
style1 = xlwt.easyxf('pattern: pattern solid, fore_colour light_green; alignment: horiz left; font: bold on, height 220; borders: left medium ' )# for dealership address
style2 = xlwt.easyxf('alignment: horiz centre; font: bold off , height 220 ')# for header
style3 = xlwt.easyxf('alignment: horiz centre; font: height 220 ')# for router output entries
worksheet.write_merge(0, 0, 0, 3, 'Automotive Group',style0)
worksheet.write_merge(1, 1, 0, 100, 'Easy Street',style1)


#***************** creating header **************************
worksheet.write(2, 0, 'Client CMF#',style2)
worksheet.write(2, 1, 'Site Name',style2)
worksheet.write(2, 2, 'Vlan',style2)
worksheet.write(2, 3, 'IP Address',style2)
worksheet.write(2, 4, 'Host Name',style2)
worksheet.write(2, 5, 'Equipmet Model(PID)',style2)
worksheet.write(2, 6, 'Cards/Modules',style2)
worksheet.write(2, 7, 'Equipment S/N',style2)
worksheet.write(2, 8, 'Interface Information (Used / Available)',style2)
worksheet.write(2, 9, 'Uplinked to (Description)',style2)
worksheet.write(2, 10, 'Circuit Detail',style2)
workbook.save(new_folder_name+'/Result.xls')
################################## read excel #################################
